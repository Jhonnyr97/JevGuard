...
function loadConfig(root = projectRoot()) {
...
// src/rules/schema.ts
var JevValue = external_exports.union([external_exports.string(), external_exports.record(external_exports.unknown()), external_exports.array(external_exports.unknown())]);
var Scope = external_exports.enum(["edit", "bash", "response", "all"]);
var Action = external_exports.enum(["block", "warn", "off"]);
var RuleBase = external_exports.object({
  instructions: JevValue,
  scope: external_exports.array(Scope).default(["all"]),
  action: Action.optional()
});
var NoulRule = RuleBase.extend({
  type: external_exports.literal("noul"),
  criteria: external_exports.object({ true: JevValue.optional(), false: JevValue.optional() }).optional(),
  pass_if: external_exports.enum(["yes", "no"]).default("yes")
});
var ChoiceRule = RuleBase.extend({
  type: external_exports.literal("choice"),
  criteria: external_exports.record(JevValue.nullable()),
  pass_options: external_exports.array(external_exports.string()).min(1)
});
var ScoreRule = RuleBase.extend({
  type: external_exports.literal("score"),
  criteria: external_exports.array(JevValue).min(2).max(10),
  min_score: external_exports.number()
});
var Rule = external_exports.discriminatedUnion("type", [NoulRule, ChoiceRule, ScoreRule]);
var RulesFile = external_exports.record(external_exports.string(), Rule);
function parseRule(id, input) {
  const raw = input;
...
// src/rules/store.ts
function rulesFilePath(root = projectRoot()) {
  const { rulesPath } = loadConfig(root);
  return isAbsolute(rulesPath) ? rulesPath : join2(root, rulesPath);
}
function loadRules(root = projectRoot()) {
  const path = rulesFilePath(root);
  if (!existsSync2(path)) return {};
  return parseRulesFile(JSON.parse(readFileSync2(path, "utf8")));
}
...
function pausePath(root = projectRoot()) {
  return join3(root, ".jevguard", "paused");
}
function isPaused(root = projectRoot()) {
  return existsSync3(pausePath(root));
}

// src/jev/client.ts
async function askJev(baseUrl, model, state, questions) {
...
// src/verify.ts
function applicableRules(rules, scope) {
...
function toQuestion(rule) {
  const criteria = "criteria" in rule ? rule.criteria : void 0;
  return { type: rule.type, instructions: rule.instructions, ...criteria !== void 0 ? { criteria } : {} };
}
function describe(instructions) {
...
async function verify(req, rules, config) {
  ...
  for (const [id, rule] of applicable) {
    ...
    const failure = { id, reason, action };
...
// src/hook.ts
var EDIT_TOOLS = /* @__PURE__ */ new Set(["Edit", "Write", "MultiEdit", "NotebookEdit", "apply_patch"]);
var BASH_TOOLS = /* @__PURE__ */ new Set(["Bash", "PowerShell"]);
...
function toolState(toolName, toolInput) {
  if (toolName && BASH_TOOLS.has(toolName)) {
    return toolInput?.command ?? toolInput;
  }
  return toolInput;
}
function buildVerifyRequest(input) {
...
function formatHookOutput(event, result) {
